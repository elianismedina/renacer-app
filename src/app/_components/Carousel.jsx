...
function Carousel () {
  ...
  const goToSlide = (/** @type {React.SetStateAction<number>} */ slideIndex) => {
    setCurrentIndex(slideIndex);
  };

  return (
    <div className='h-[780px] w-full m-auto py-10 px-0 relative group sm:py-6'>
      <div
        style={{ backgroundImage: `url(${slides[currentIndex]?.url})` }}
        className='w-full h-full rounded-none bg-center bg-cover duration-500'
      ></div>
      {/* Left Arrow */}
      <div className='hidden group-hover:block absolute top-[50%] -translate-x-0 translate-y-[-50%] left-5 text-2xl rounded-full p-2 bg-violet-600 text-white cursor-pointer'>
